...
class DecisionTree:
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        n_labels = len(np.unique(y)) if self.task == 'classification' else 2 
    ...
    def _calculate_gain(self, y, X_column, threshold):
        ...
        if self.task == 'classification':
            ...
            child_loss = p * self._entropy(y[left_idxs]) + (1 - p) * self._entropy(y[right_idxs])
        else:
            parent_loss = np.var(y)
            child_loss = p * np.var(y[left_idxs]) + (1 - p) * np.var(y[right_idxs])
